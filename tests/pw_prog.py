...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await pg.goto("http://127.0.0.1:8749/player.html")
        ...
        # 最后一个同行小节 -> 扫到行尾
        r=await prog(6.5)
        # 行末小节按同行中位间距（.20）收边，不再一路铺到页面右缘
        print(ok(abs(r["L"]-.75)<.005 and abs(r["L"]+r["W"]-.95)<.008),
              f"小节4（下一小节换行）: 带子收在 x={r['L']}→{r['L']+r['W']:.3f} (期望 .75→.95，同行间距 .20)")
        r=await prog(8.2)
