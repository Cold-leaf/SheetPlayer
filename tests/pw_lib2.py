...
async def main():
    ...
    async with async_playwright() as p:
        ...
        h1=await pg.evaluate("pdfHash"); pid1=await pg.evaluate("pid"); name1=await pg.evaluate("track.name")
        await pg.set_input_files("#fPdf",PDF2)   # 内容相同、名字不同
        # 旧页面的 dataset.done/M 在 resetTrack 前仍存在，等「加载完成 + 恢复完成」才不算竞态
        # 旧页面的 dataset.done/M 在加载前仍存在，等「新文件名 + 恢复完成」才不算竞态
        await pg.wait_for_function("()=>pdfName==='换个名字.pdf'&&M.length===3",timeout=30000)
        print(ok(await pg.evaluate("M.length")==3), "换个文件名导入同一份 PDF：标注按内容哈希认领保留")
        await pg.evaluate("$('bLib').onclick()"); await pg.wait_for_timeout(500)
        print(ok(await pg.evaluate("document.querySelectorAll('.libCard').length")==1), "内容去重：项目没有重复建")
        # 身份是项目：换文件名只把附件名换掉，项目名不动（要改得自己点「改名」）
        print(ok(await pg.evaluate("pid")==pid1 and await pg.evaluate("track.name")==name1),
              "同一个项目、名字不被文件名带着跑: "+name1)
        ...
        print(ok(j["app"]=="sheetplayer" and j["v"]==2 and len(j["items"])==1 and
                 j["items"][0]["data"]["M"] and len(j["items"][0]["data"]["M"])==3),
              "批量导出包含标注数据（v2）")
        print(ok(str(j["items"][0]["id"]).startswith("p") and j["items"][0]["pdf"]==h1 and
                 j["items"][0]["pdfHistory"]==[] and "aka" in j["items"][0]),
              "导出条目带 id / pdf / aka / pdfHistory: "+
              json.dumps({k:v for k,v in j["items"][0].items() if k not in ("data","pdf")},ensure_ascii=False))
        print(ok(j["items"][0]["name"]==name1), "导出条目带项目名: "+j["items"][0]["name"])
        ...
        print(ok(await pg.evaluate("""(async()=>{const db=await new Promise(r=>{const o=indexedDB.open('sheetplayer');o.onsuccess=()=>r(o.result)});
          const n={};
          for(const s of ['projects','marks','files','tracks','anns'])n[s]=await new Promise(r=>{db.transaction(s).objectStore(s).getAll().onsuccess=e=>r(e.target.result.length)});
          return n.projects===0&&n.marks===0&&n.files===0})()""")), "级联删除：projects/marks/files 全清")
