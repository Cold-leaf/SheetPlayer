...
- **曲目库**（IndexedDB，库名 `sheetplayer`）：标注按 PDF **内容哈希**（前 1MB SHA-256）关联，文件名只做显示；PDF/音频文件副本、频谱分析缓存也在库里。换文件名不丢标注；同一内容重复导入自动去重；谱子只加了笔记重新导出（内容变、名字不变）会自动按文件名接回原曲目，时间点与竖线照旧。
...
- **跨设备同步（推荐）**：标注走 GitHub 仓库同步，文件留在各设备本地。
  1. 桌面（标注的主设备）：「导出全部标注」下载 JSON。
  2. 打开仓库网页 → 拖拽上传覆盖根目录的 `annotations.json` → commit。
  3. 平板/手机：曲目库点「**从 GitHub 同步**」→ 自动拉取合并（按时间戳保留较新的，本地较新的不会被旧数据覆盖）。
  - 只同步标注（几十 KB），不搬文件——对端把 PDF/音频导进库后按哈希自动对上。
  - 注意：仓库是公开的，`annotations.json` 里的练习标注也是公开可见的。
