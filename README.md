...
其他能力：自动生成时间轴（按拍号/播放顺序）、半自动标小节（点行首补齐整行）、小节线吸附、频谱与起音吸附、多小节休止、横向铺开、练习循环、**音频模式**（同一谱子多套时间点：MIDI 版 / 现场版各一套）、**播放列表**（选定曲目连播，可指定音频版本、循环、每首后暂停）、触屏适配、双指/触控板缩放、JSON 导入导出、自动存档。
